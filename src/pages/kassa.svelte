<script>
	import { api, cart, data } from '../components/store';
	...
	async function sendSubmit(event) {
		if (typeof paymentSelected !== 'undefined') {
			$cart.method = paymentSelected;
			cart.set($cart);
		}
		...
		inputs.forEach(function (value) {
			if (typeof event.target[value.name] !== 'undefined')
				formData.append(value.name, event.target[value.name].value);
		});

		// Rest of the fields for payment
